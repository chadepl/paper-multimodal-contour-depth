...
if __name__ == "__main__":
    ...
    # depth computation: (slow, fast)
    
    t_tick = time()
    depths_slow = depth_fun(masks, modified=True, fast=False)
    print(f"Slow depths ran in {time() - t_tick} seconds")

    t_tick = time()
    depths_fast = depth_fun(masks, modified=True, fast=True)
    ...
    depths_slow = (depths_slow - depths_slow.min())/(depths_slow.max() - depths_slow.min())
    depths_fast = (depths_fast - depths_fast.min())/(depths_fast.max() - depths_fast.min())
